  ...
  <div class="privileges-table">
    ...
    <el-table :data="filterTableData"
      ...
      <el-table-column prop="tag"
        label="标签"
        width="100"
        :filters="[{ text: '运营部', value: '运营部' }, { text: '技术部', value: '技术部' }]"
        :filter-method="filterTag"
        filter-placement="bottom-end">
        <template slot-scope="scope">
          <el-tag :type="scope.row.tag === '运营部' ? 'primary' : 'success'"
    ...
    <el-dialog :title="title"
      ...
      <Dialog :edit-data="rowList"
        @change="changeStatus"
        @upData="updata"></Dialog>
    </el-dialog>
    ...
    filterTableData () {
      if (this.search.length > 0) {
        return this.tableData.filter(it => {
          return (
            it.type.indexOf(this.search) !== -1 ||
            it.department.indexOf(this.search) !== -1
          );
        });
      } else {
